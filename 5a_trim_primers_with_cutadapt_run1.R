# R script to search for and remove primers from mel's metabarcoding round 1

##### Command syntax is okay, and script runs,
##### but step 3 is not producing any trimming results... not sure why?


raw_dir <- "/vlsci/VR0267/pgriffin/hsm/mel_metabarcoding_run1/raw/"
trimmed_dir <- "/vlsci/VR0267/pgriffin/hsm/mel_metabarcoding_run1/trimmed/"
pc_trimmed_threshold <- 0.5
adapter_table <- read.csv("/vlsci/VR0267/pgriffin/hsm/mel_metabarcoding_run1/scripts/primer_sequences_for_R_script.csv", 
                    header=TRUE, stringsAsFactors=FALSE)
raw_files <- list.files(path=raw_dir, pattern="R1_001.fastq$")

for(each_file in raw_files){
  
  basename <- strsplit(each_file, split="_R1_001.fastq")[[1]][1]
  # start with a raw R1 and R2 file
  
  input_R1<-paste(raw_dir, basename, "_R1_001.fastq", sep="")
  input_R2<-paste(raw_dir, basename, "_R2_001.fastq", sep="")
  ...
  for(i in 1:nrow(adapter_table)){
    ...
    if(pc_trimmed < pc_trimmed_threshold){
      ...
      # move to P2f on untrimmed output from STEP 3 (STEP 6)
      # Actually, decided to start from quality-trimmed file for each new adapter pair
      # because of the cases where primer B occurs in >1 adapter pair
      #input_R1 <- paste(trimmed_dir, basename, step3_stepcode, "_untrimmed_R1.fastq", sep="")
      #input_R2 <- paste(trimmed_dir, basename, step3_stepcode, "_untrimmed_R2.fastq", sep="")
